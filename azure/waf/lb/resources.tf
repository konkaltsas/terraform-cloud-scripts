resource "azurerm_public_ip" "terraform-ubuntu-public-ip" {
  name                = format("tf-backend-service-public-ip-node-%v", count.index)
...
resource "azurerm_network_interface" "terraform-ubuntu-management-interface" {
  name                = format("tf-backend-service-management-interface-node-%v", count.index)
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_network_interface" "terraform-ubuntu-server-interface" {
  name                = format("tf-backend-service-server-interface-node-%v", count.index)
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_linux_virtual_machine" "terraform-ubuntu-machine" {
  name                = format("tf-backend-service-machine-node-%v", count.index)
  resource_group_name = var.resource_group_name
  location            = var.location
  size                = var.vm_size
...
resource "citrixadc_lbvserver" "terraform_lb" {
...
resource "citrixadc_servicegroup" "ubuntu_servers" {
  ...
  lbvservers       = [citrixadc_lbvserver.terraform_lb.name]
  servicetype      = "HTTP"
  clttimeout       = "40"
...
resource "citrixadc_nsconfig_save" "tf_ns_save_lb" {
    all        = true
    timestamp  = "2020-06-20T12:37:06Z"

    depends_on = [
      citrixadc_lbvserver.terraform_lb,
      citrixadc_servicegroup.ubuntu_servers
    ]
}